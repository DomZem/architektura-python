...
class TestProduct(unittest.TestCase):
    ...
    def test_is_not_available_when_empty(self):
        ...
        product_empty = Product("Mysz", 99.99, 0)
        self.assertFalse(product_empty.is_available())

    # --- Testy total_value ---

    def test_total_value(self):
        """Sprawdz, czy total_value zwraca price * quantity."""
        self.assertAlmostEqual(self.product.total_value(), 29999.90)

    # --- Testy apply_discount ---
    def test_apply_discount(self):
        self.product.apply_discount(10)
        self.assertAlmostEqual(self.product.price, 2699.991)

    def test_apply_discount_invalid_raises(self):
        with self.assertRaises(ValueError):
            self.product.apply_discount(-10)
        with self.assertRaises(ValueError):
